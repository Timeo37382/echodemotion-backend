...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    const { data, ok } = await sb('reviews?select=*&order=created_at.desc');
    return ok ? res.json(data) : res.status(500).json({ error: 'Erreur' });
  }
  if (req.method === 'PATCH') {
    const { id, ...updates } = req.body;
    const { data, ok } = await sb(`reviews?id=eq.${Number(id)}`, { method: 'PATCH', body: JSON.stringify(updates) });
    return ok ? res.json(data) : res.status(500).json({ error: 'Erreur' });
  }
  if (req.method === 'DELETE') {
    const { id } = req.body;
    const { ok } = await sb(`reviews?id=eq.${Number(id)}`, { method: 'DELETE' });
